(function () {
    ...
    //extra functionality, trying to present different shape to different class
    let dataShape = d3.scaleOrdinal().range(["circle"]);
    ...
    let divPrefix = '';
    /**
     * locate the data node in D3 canvas
     * input parameters:
     *   tempData: detaframe;
    ...
    function locateNode(tempData, featureNames, features) {
        ...
        tempData.forEach(function (d) {
            ...
            featureNames.forEach(function (k, i) {
                dx += Math.cos(features[i]) * d[k];
                dy += Math.sin(features[i]) * d[k];
            }); // dx & dy
            d.x0 = dx / dsum;
            d.y0 = dy / dsum;
    ...
    function normailzeData(data) {
        ...
        let scales = {};
        features.forEach(function (dimension) {
            scales[dimension] = d3.scaleLinear().domain(d3.extent(data.map(function (d, i) {
                return d[dimension];
            }))).range([0, 1]);
        });
        data.forEach(function (d) {
            features.forEach(function (dimension) {
                d[dimension + suffix] = scales[dimension](d[dimension]);
            });
        });
        data.forEach(function (d) {
            let dsum = 0;
    ...
    function init(data,div_mainpanel) {
        ...
        //initlize the feature node coordinate
        ...
        dataAfter.forEach((d, i) => {
            ...
            d.shape = dataShape(d[target]);
        });

        dataAfter = normailzeData(dataAfter);
        dataAfter = locateNode(dataAfter, featureNames, featureAnchor); // locateNode. need update when DAs move.

        console.log("d.color:", dataAfter);

        // prepare the features data
        let featureNodes = features.map(function (d, i) {
            return {
                theta: featureAnchor[i], //[0, 2*PI]
                x: Math.cos(featureAnchor[i]) * radius + radius,
                y: Math.sin(featureAnchor[i]) * radius + radius,
                fixed: true,
                name: d
            };
        });

        let svg = mainPanel.append('svg').attr('id', 'radviz')
            .attr('width', width)
            .attr('height', height);
        ...
        function renderData() {
            function chart(div) {
                div.each(function () {
                    ...
                    /**
                     * step 1: prepare feature node on canvas
                     */
                        ...
                        center.selectAll('circle.featureNodes')
                            ...
                            .call(d3.drag()
                                ...
                                .on('drag', function (d, index) {
                                    ...
                                    locateNode(dataAfter, featureNames, featureAnchor);
                                    prepareDataNodes();
                                })
                                .on('end', function (d) {
                    ...
                    /**
                     * step 2: prepare actual data node on convas
                     */
